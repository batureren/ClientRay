// components/dialogs/AccountHistoryDialog.jsx
...
const AccountHistoryDialog = ({ open, onOpenChange, account, history, isLoading, formatDate, getActionIcon }) => {
  const { t } = useTranslation()
  const [activeTab, setActiveTab] = useState('history')

  const getActionColor = (actionType) => ({
    'created': 'bg-green-100 text-green-800', 'updated': 'bg-blue-100 text-blue-800',
    'product_assigned': 'bg-purple-100 text-purple-800', 'product_updated': 'bg-orange-100 text-orange-800',
    'product_removed': 'bg-red-100 text-red-800', 'task_created': 'bg-cyan-100 text-cyan-800',
  ...
  const summaryStats = [
    { label: t('accountHistory.summary.created'), color: 'green-600', key: 'created' },
    { label: t('accountHistory.summary.updated'), color: 'blue-600', key: 'updated' },
    { label: t('accountHistory.summary.productsAssigned'), color: 'purple-600', key: 'product_assigned' },
    { label: t('accountHistory.summary.productsUpdated'), color: 'orange-600', key: 'product_updated' },
    { label: t('accountHistory.summary.tasksCreated'), color: 'cyan-600', key: 'task_created' },
  ]
      ...
      <DialogContent className="max-w-6xl w-[95vw] max-h-[90vh] h-[90vh] overflow-hidden p-3 sm:p-6">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="flex items-center gap-2 text-base sm:text-lg">
            ...
            <span className="truncate">{t('accountHistory.title', { name: account.account_name })}</span>
          </DialogTitle>
                ...
                <div className="pb-4">
                  {isLoading ? (
                    <div className="flex items-center justify-center py-8">
                      ...
                      <span className="ml-3 text-gray-600 text-sm sm:text-base">{t('accountHistory.loading')}</span>
                    </div>
                  ) : history.length === 0 ? (
                    <div className="text-center py-8 text-gray-500 text-sm sm:text-base">{t('accountHistory.noHistory')}</div>
                  ) : (
                              ...
                              <div className="flex items-start gap-3 flex-1 min-w-0">
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex flex-wrap items-center gap-2 mb-2">
                                    <Badge className={getActionColor(entry.action_type)}>{t(`accountHistory.actionTypes.${entry.action_type}`, { defaultValue: entry.action_type.replace(/_/g, ' ').toUpperCase() })}</Badge>
                                    <span className="text-sm text-gray-500">{t('accountHistory.byUser', { name: entry.user_name || t('accountHistory.unknownUser') })}</span>
                                  </div>

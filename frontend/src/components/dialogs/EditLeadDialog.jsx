// components/dialogs/EditLeadDialog.jsx
...
// Lead Status Progress Component (Arrow Style)
...
const EditLeadDialog = ({ open, onOpenChange, lead, onLeadUpdated }) => {
  const { t } = useTranslation();
  ...
  const populateForm = (leadData, fields) => {
    const initialCustomFields = {};
    fields.forEach(field => {
      const leadCustomValue = leadData.custom_fields?.find(cf => cf.field_name === field.field_name);
      
      // Handle different field types for initial values
      if (field.field_type === 'BOOLEAN') {
        initialCustomFields[field.field_name] = leadCustomValue?.value ?? false;
      } else if (field.field_type === 'MULTISELECT') {
        // Parse MULTISELECT values - they should be stored as JSON arrays
        let multiSelectValue = [];
        if (leadCustomValue?.value) {
          try {
            multiSelectValue = typeof leadCustomValue.value === 'string' 
              ? JSON.parse(leadCustomValue.value) 
              : leadCustomValue.value;
          } catch (e) {
            console.warn(`Failed to parse multiselect value for ${field.field_name}:`, leadCustomValue.value);
            multiSelectValue = [];
          }
        }
        initialCustomFields[field.field_name] = Array.isArray(multiSelectValue) ? multiSelectValue : [];
      } else {
        initialCustomFields[field.field_name] = leadCustomValue?.value ?? '';
      }
    });
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await api.put(`/leads/${lead.id}`, leadForm);
      onLeadUpdated();
    } catch (error) {
  ...
  const renderCustomField = (field, disabled = false) => {
    const value = leadForm.custom_fields ? leadForm.custom_fields[field.field_name] : '';
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t('editLeadDialog.title')}</DialogTitle>
          <DialogDescription>{t('editLeadDialog.description')}</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4 max-h-[75vh] overflow-y-auto p-1">
          <div className="pb-4 border-b">
            <Label className="text-base font-semibold">{t('editLeadDialog.labels.status')}</Label>
            <div className="mt-2">
              {leadForm.status === 'converted' ? (
                <div className="p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 rounded-md">
                  <span className="text-green-700 font-semibold">✓ {t('editLeadDialog.convertedStatus')}</span>
                </div>
              ) : ( <LeadStatusProgress currentStatus={leadForm.status} onStatusChange={handleStatusChange} /> )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="email">{t('createLeadDialog.labels.email')}</Label>
              <Input id="email" type="email" value={leadForm.email} onChange={(e) => handleChange('email', e.target.value)} required />
            </div>
            <div>
              <Label htmlFor="phone">{t('createLeadDialog.labels.phone')}</Label>
              <Input id="phone" value={leadForm.phone} onChange={(e) => handleChange('phone', e.target.value)} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="company">{t('createLeadDialog.labels.company')}</Label>
              <Input id="company" value={leadForm.company} onChange={(e) => handleChange('company', e.target.value)} />
            </div>
            <div>
              <Label htmlFor="website">{t('createLeadDialog.labels.website')}</Label>
              <Input id="website" value={leadForm.website} onChange={(e) => handleChange('website', e.target.value)} />
            </div>
          </div>
          <div>
            <Label htmlFor="address_line1">{t('createLeadDialog.labels.address')}</Label>
            <Input id="address_line1" value={leadForm.address_line1} onChange={(e) => handleChange('address_line1', e.target.value)} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="city">{t('createLeadDialog.labels.city')}</Label>
              <Input id="city" value={leadForm.city} onChange={(e) => handleChange('city', e.target.value)} />
            </div>
            <div>
              <Label htmlFor="state">{t('createLeadDialog.labels.state')}</Label>
              <Input id="state" value={leadForm.state} onChange={(e) => handleChange('state', e.target.value)} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="postal_code">{t('createLeadDialog.labels.postalCode')}</Label>
              <Input id="postal_code" value={leadForm.postal_code} onChange={(e) => handleChange('postal_code', e.target.value)} />
            </div>
            <div>
              <Label htmlFor="country">{t('createLeadDialog.labels.country')}</Label>
              <Input id="country" value={leadForm.country} onChange={(e) => handleChange('country', e.target.value)} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="lead_source">{t('createLeadDialog.labels.leadSource')}</Label>
              <Input id="lead_source" value={leadForm.lead_source} onChange={e => handleChange('lead_source', e.target.value)} />
            </div>
            <div>
              <Label htmlFor="notes">{t('createLeadDialog.labels.notes')}</Label>
              <Textarea id="notes" value={leadForm.notes} onChange={e => handleChange('notes', e.target.value)} rows={3} />
            </div>
          </div>
